`timescale 1ns/1ps
// Dynamic-beta, low-resource MARG attitude estimator for xc7z020clg400-2.
//
// This is an intentionally independent variant of rtl/marg_estimator_core.sv.
// It preserves the fixed-beta core unchanged and adds a hardware-oriented
// confidence gate before Eq. (11).  The nominal beta still arrives through
// beta_in; the gate selects {0, beta0/4, beta0/2, beta0} after a hysteretic
...
module marg_estimator_dynamic_beta_core #(
    ...
    // Unit a/m, SAAM, gyro prediction, and persistent quaternion state.
    ...
    always_comb begin
        ...
        // A hard violation rejects SAAM.  Otherwise one soft violation uses
        // beta0/2 and two or more use beta0/4.  All thresholds compare raw
        // (pre-normalization) squared norms, except mN2 and innovation.
        ...
        if (!rst_n) begin
            ...
            case (state)
                ...
                C_GYRO_ONLY_PREP: begin
                    // A missing/degenerate reference observation is a
                    // hard rejection for this frame.  The quaternion still
                    // follows the established gyro-only fallback path.
                    beta_level_pending <= 2'd0;
                    // Use the configurable confirmation count rather than a
                    // literal so the gyro-only fallback follows the same
                    // policy as the normal observation path.
                ...
                C_ALIGN_DECIDE: begin
                    ...
                    // Confirm degradation only after BAD_FRAME_COUNT
                    // consecutive observations.  Recovery moves one
                    // discrete level after GOOD_FRAME_COUNT clean frames.
                    // Pending state is committed only on output handshake.
                    if (candidate_raw_level < 2'd3) begin
                        ...
                        if (bad_frame_count_l >= (BAD_FRAME_COUNT - 1)) begin
                            // An abnormal observation may lower the current
                            // correction weight after confirmation, but it
                            // must never raise it.  Raising beta is reserved
                            // for the clean-frame recovery path below, so an
                            // intermittent soft anomaly cannot bypass the
                            // GOOD_FRAME_COUNT hysteresis requirement.
