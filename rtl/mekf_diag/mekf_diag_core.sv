`timescale 1ns/1ps
// Exploratory isotropic-diagonal 6-state multiplicative EKF (MEKF).
//
// Nominal state: q(body->world), gyro bias b.  Error state: dtheta, db.
// Covariance model: P=diag(Ptheta*I3, Pb*I3), deliberately not a full 6x6
// MEKF.  It is a resource-bounded Kalman-type baseline, not a substitute for
// a full covariance EKF.  All normalizations are conventional sqrt/divide;
// no SAAM solver or rational-normalization logic appears in this module.
module mekf_diag_core #(
    ...
    always_ff @(posedge clk) begin
        ...
        if(!rst_n) begin
            ...
            // CORR_START already multiplies each small-angle component by
            // 1/2.  Applying q30_half again here would incorrectly make the
            // quaternion correction dtheta/4 rather than dtheta/2.
