`timescale 1ns/1ps
// Conventional x/sqrt(x'x) normalizer with one time-shared multiplier, one
// restoring square root and one restoring divider.  Unlike the SAAM project,
// it neither instantiates nor approximates the rational normalization law.
module standard_normalizer_q30_shared (
    input  logic              clk,
    input  logic              rst_n,
    input  logic              start,
    input  logic [2:0]        dim,
    ...
    typedef enum logic [3:0] {
        IDLE, SQUARE_START, SQUARE_WAIT, SQRT_START, SQRT_WAIT,
        DIV_START, DIV_WAIT, FINISH
    } state_t;
    state_t state;
    ...
    always_comb begin
        case (component)
            3'd0: active_component = v0_reg;
            3'd1: active_component = v1_reg;
            3'd2: active_component = v2_reg;
            default: active_component = v3_reg;
        endcase
        norm_sum_next = norm2_q60 + $unsigned(mul_product);
        mul_start = (state == SQUARE_START);
        mul_a = active_component;
        mul_b = active_component;
        sqrt_start = (state == SQRT_START);
        div_start = (state == DIV_START);
        div_dividend = {2'b00, abs_s32(active_component), 30'b0};
        div_divisor = norm_q30;
    end

    standard_mul_signed32_seq u_multiplier (
    ...
    standard_div_u64_u32_seq u_div (
        .clk(clk), .rst_n(rst_n), .start(div_start), .dividend(div_dividend),
        .divisor(div_divisor), .busy(div_busy), .done(div_done),
        .div_by_zero(div_by_zero), .quotient(div_quotient)
    );
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            norm2_q60 <= '0; norm2_q30 <= '0; norm_q30 <= '0;
            zero_input <= 1'b0;
            ...
            case (state)
                ...
                SQUARE_WAIT: if (mul_done) begin
                    norm2_q60 <= norm_sum_next;
                    if (component + 3'd1 >= dim_reg) begin
                        norm2_q30 <= (|norm_sum_next[63:62]) ? 32'hffff_ffff : norm_sum_next[61:30];
                        state <= SQRT_START;
                    end else begin
                ...
                DIV_WAIT: if (div_done) begin
                    case (component)
                        3'd0: v0_out <= signed_quotient_q30(div_quotient, active_component[31]);
                        3'd1: v1_out <= signed_quotient_q30(div_quotient, active_component[31]);
                        3'd2: v2_out <= signed_quotient_q30(div_quotient, active_component[31]);
                        default: v3_out <= signed_quotient_q30(div_quotient, active_component[31]);
                    endcase
