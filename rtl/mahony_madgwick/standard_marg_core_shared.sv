`timescale 1ns/1ps
// Resource-shared conventional MARG baselines.
//
// FILTER_MODE=0: Mahony PI complementary observer.
// FILTER_MODE=1: Madgwick normalized-gradient observer.
//
// Both variants use conventional x/sqrt(x'x) normalization and a standard
// horizontal-field square root.  The only reused arithmetic primitive is one
// registered general multiplier; no SAAM determination or rational
// normalization is present in this source tree.
module standard_marg_core_shared #(
    ...
    // Gyroscope samples remain Q24 throughout the angular-rate path.  The
    // previous implementation converted them to Q30, whose signed 32-bit
    // range is only [-2, 2) rad/s, and therefore silently saturated fast
    // motion.  Mahony correction terms are converted in the opposite
    // direction (Q30 -> Q24) before being added to the raw rate.
    ...
    // One conventional normalizer is time-shared among a, m, grad and q.
    always_comb begin
    ...
    // Micro-sequenced operand selection.  Each START state issues exactly one
    // conventional signed multiply; each WAIT state stores its rounded result.
    always_comb begin
        ...
        case (state)
            ...
            QDOT_START: begin
                // q is Q30 and angular rate is Q24.  Store the derivative in
                // Q27 (rather than Q30) so a 32-bit signed word covers
                // [-16,16) s^-1.  DT_START subsequently multiplies Q30 dt by
                // this Q27 derivative and shifts by 27 to recover Q30 delta-q.
        ...
        if (!rst_n) begin
            ...
            case (state)
                ...
                BETA_CALC: begin
                    ...
                    // qdot is Q27 here; beta*gradient is Q30.
