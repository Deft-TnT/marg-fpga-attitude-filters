`timescale 1ns/1ps
// Conventional MARG observer core shared by the independent Mahony and
// Madgwick baselines.  FILTER_MODE=0 selects Mahony PI feedback; FILTER_MODE=1
// selects Madgwick's normalized gradient descent.  No SAAM observation or
// rational normalization is instantiated.
module standard_marg_core #(
    ...
    // Current-frame algebraic intermediates.  Every value is signed Q30,
    // except gyroscope input words, which retain their Q24 representation.
    ...
    always_comb begin
        // The normalizer is exclusively the conventional sqrt/divide block.
        norm_start = 1'b0;
        norm_dim = 3'd3;
    ...
    // Quaternion-to-rotation terms, predicted reference vectors, observer
    // residuals, and both standard update laws.
    always_comb begin
        ...
        // Madgwick objective: gravity residual and, when available, magnetic
        // residual.  The normalized J^T f direction is algebraically the
        // usual MARG gradient descent update without copying SAAM logic.
