`timescale 1ns/1ps
// Three-mode v2, low-resource MARG attitude estimator for xc7z020clg400-2.
//
// This is an intentionally independent extension of the fixed-beta and
// dynamic-beta-v1 cores.  Neither earlier implementation is modified.
//
// The reference-observation policy has three modes:
//   MARG      -- use the SAAM quaternion with the v1 confidence-gated beta;
//   IMU_TILT  -- reject magnetic heading, construct a gravity-aligned,
//                world-vertical-twist-preserving quaternion reference; and
//   GYRO_ONLY -- reject the gravity reference and propagate with the gyro.
//
// Reliability revision relative to the frozen v1 evidence:
//   * a hard predicted-gravity residual independently rejects the
//     accelerometer and has priority over IMU_TILT;
//   * a hard horizontal-heading residual independently rejects magnetic
//     heading, so direction-only magnetic disturbances enter IMU_TILT;
//   * simultaneous hard gravity/heading residuals resolve to GYRO_ONLY;
//
// Prediction residuals are deliberately built only from the existing shared
// sequential multiplier and Eq. (12) normalizer.  No CORDIC, atan2, extra
// multiplier, divider or square-root is introduced.  This favours latency over
// DSP area and makes the eventual fixed/dynamic/three-mode resource comparison
// meaningful on the same Zynq-7000 part.
//
// Algorithmic contract
//   * a/m inputs are calibrated direction candidates in S(32,30); this core
//     applies Eq. (12) before SAAM Eq. (5)/(6).
//   * omega inputs are S(32,24), in rad/s. dt and beta are U(32,30).
//   * q is scalar-first [w,x,y,z], S(32,30).
//   * m_N uses one deterministic integer square-root (Eq. (5)).
//   * A single sequential normalizer is time-shared by a, m, q_SAAM,
//     q_k,omega and q_k.
//   * A second single multiplier is shared by all SAAM, gyro, dot and fusion
//     operations.  Latency is intentionally prioritised over area/energy.
module marg_estimator_three_mode_core #(
    // U(32,30) seconds.  20 ms covers common 50 Hz and faster IMUs while
    // retaining a small Euler propagation step for the intended sensor range.
    // Board-specific wrappers may override it for a known sampling period.
    parameter logic [31:0] MAX_DT_Q30 = 32'd21_474_836,

    // Simulation-default confidence thresholds.  They are all UQ30 values
    // and are parameters so board calibration can replace them later.
    ...
    // Direction residual thresholds.  They compare unit-vector dot products
    // and therefore require no inverse trigonometric operation.
    ...
    // cos^2(delta_heading), evaluated without atan2 or division from the
    // qgyro/qSAAM horizontal-heading pairs.  The current values correspond
    // to approximately 1.5 deg (soft) and 6 deg (hard), respectively; both
    // are calibration parameters rather than universal physical constants.
    ...
    // IMU tilt correction is beta0/4 by default; it remains tied to the
    // runtime beta0 rather than becoming an uncalibrated magic constant.
    parameter integer IMU_BETA_SHIFT = 2,
    // cT^2 and rho^2 must be nonzero to define roll/pitch and ZYX heading.
    ...
    // Informational only: bits [6:0] retain the fixed-core error meanings.
    ...
    typedef enum logic [6:0] {
        ...
        // Prediction-direction residuals.  The qgyro terms are shared by
        // gravity consistency, yaw extraction and the IMU reference.
        ...
        // Sign alignment with qSAAM supplies the innovation residual.
        C_ALIGN_INIT, C_ALIGN_MUL_START, C_ALIGN_MUL_WAIT, C_ALIGN_DECIDE,
        C_MODE_DECIDE, C_GYRO_ONLY_PREP,
        // Strict-ZYX-heading IMU reference.  qacc is composed as
        // qyaw(qgyro) * qpitch(accel) * qroll(accel), then Eq. (12) normalized.
    ...
    // Unit a/m, SAAM, gyro prediction, IMU tilt reference, and persistent
    // quaternion state.
    ...
    // Confidence-gate state.  Levels encode reject=0, quarter=1,
    // half=2, nominal=3.  The raw level is recomputed per frame while the
    // committed level is hysteretic and changes only after consecutive data.
    ...
    // Three-mode state.  A source-mode transition is hysteretic and is
    // committed atomically with qprev.  MODE_MARG < MODE_IMU_TILT <
    // MODE_GYRO_ONLY corresponds to decreasing use of absolute references.
    ...
    // Direction-residual and strict-heading IMU-reference intermediates.
    // All components below use S(32,30), except *_product/accumulators Q60.
    ...
    // The IMU reference is constructed as yaw(qgyro)*pitch(a)*roll(a).
    // Each pair is deliberately unnormalised; their common positive scales
    // are removed by the one final Eq. (12) quaternion normalisation.
    ...
    always_comb begin
        ...
        // The sign-alignment dot product is already available in Q60.  Its
        // magnitude is also the zero-extra-multiplier innovation metric.
        dot_abs_q60 = dot_sum[63] ? $unsigned(-dot_sum) : $unsigned(dot_sum);
        qdot_abs_q30 = rshift_u64_rne(dot_abs_q60, 30);

        // A hard violation selects a lower source mode.  In MARG mode, one
        // soft violation uses beta0/2 and two or more use beta0/4.  Raw
        // sensor norms retain the pre-normalization magnitude information;
        // r_a/r_m are directional residuals calculated after qgyro is valid.
        ...
        if (accel_unit_valid && (accel_dir_dot_q30 < ACC_DIR_NORMAL_Q30)) begin
            ...
            if (accel_dir_dot_q30 < ACC_DIR_REJECT_Q30) begin
                accel_dir_hard = 1'b1;
                // V2: direction-only linear acceleration can preserve a unit
                // norm through cancellation with gravity.  A hard residual is
                // therefore independently actionable.  The mode priority
                // below guarantees that it selects GYRO_ONLY even when the
                // magnetic residual simultaneously requests IMU_TILT.
        ...
        if (mag_unit_valid && saam_available &&
            ...
            if (mag_dir_hard) begin
                // V2: magnetic direction can be corrupted while its norm is
                // unchanged.  A hard horizontal-heading residual therefore
                // independently requests IMU_TILT.  If gravity is also hard,
                // the accelerometer-first priority below selects GYRO_ONLY.
                mag_direction_actionable = 1'b1;
                candidate_raw_level = 2'd0;
            end
            else begin
                mag_dir_soft = 1'b1;
                // A direction-only magnetic fault can keep |m| and m_N^2
                // inside their normal bands.  Require concurrent quaternion
                // innovation before acting on a soft heading residual.  This
                // catches the documented weak-magnetic blind spot without
                // making a single soft direction sample sufficient by itself.
    ...
    // The source mode used for the current transaction is a pure function of
    // the committed mode and this frame's already-computed evidence.  The
    // same condition is used below to update the pending, atomic state.
    always_comb begin
        ...
        // Do not let a hard source-quality failure contaminate even the
        // first two frames of the normal three-frame confirmation window.
        // The persistent mode register still changes only through the
        // hysteretic pending-state path, but this transaction fails safe
        // immediately.
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            case (state)
                ...
                // Preserve raw sensor magnitude information before the
                // required Eq. (12) vector normalizations discard it.  The
                // existing sequential multiplier is reused for all six
                // squares, so this adds cycles but no multiplier resource.
                ...
                C_SAAM_NORM_START: begin
                    if (!norm_busy) begin
                        norm_dim3 <= 1'b0;
                        // Paper-mandated reorder: [q_tilde3,q_tilde0,q_tilde1,q_tilde2].
                        norm_in0 <= qtilde3;
                        norm_in1 <= qtilde0;
                        norm_in2 <= qtilde1;
                        norm_in3 <= qtilde2;
                    ...
                    if (norm_done) begin
                        if (norm_zero) begin
                            ...
                            if (have_state)
                                ...
                                // First valid frame initializes from absolute SAAM.
                ...
                C_GYRO_FINAL: begin
                    // q_k,omega = (I + dt/2 Omega) q_(k-1), scalar first.
                ...
                // This reuses the same Eq. (12) hardware.  It prevents the
                // first-order Euler propagation from changing the effective
                // fusion weight through norm drift, without adding arithmetic
                // resources.
                ...
                // qgyro is now normalised.  Extract the nine reusable
                // quaternion products required by g_pred and by the yaw
                // projection.  The sequential multiplier is intentionally
                // reused: this adds latency but no DSP instance.
                ...
                // The horizontal magnetic-direction residual compares the
                // yaw-plane pairs of qgyro and qSAAM.  It is equivalent to
                // an inclination-compensated heading consistency test, but
                // avoids atan2, a world-field lookup and a second square root.
                ...
                // Dynamic-beta-v1 is retained as the MARG-mode subpolicy.
                // The separate mode state decides whether the source itself
                // is MARG, the strict-heading IMU reference, or gyro-only.
                C_MODE_DECIDE: begin
                    ...
                    if (mode_decided_comb == MODE_MARG) begin
                        ...
                        // The absolute magnetic heading is unavailable in
                        // this mode.  Reset (rather than freeze) the MARG
                        // gain state so that a later return cannot apply the
                        // pre-fault full beta to an only momentarily clean
                        // SAAM observation.  MARG re-entry must pass through
                        // the existing staged recovery 0 -> 1/4 -> 1/2 -> 1.
                        ...
                        // Both absolute references are rejected; any future
                        // MARG correction must likewise restart at beta=0.
                        beta_level_pending <= 2'd0;
                        bad_frame_count_pending <= '0;
                        good_frame_count_pending <= '0;
                        beta_active_l <= '0;
                ...
                // cT = sqrt(a_y^2+a_z^2) and rho = sqrt(X_g^2+Y_g^2).
                // The two roots reuse the same m_N square-root hardware.
                C_IMU_CT_INIT: begin
                    dir_sum <= '0;
                    op_index <= '0;
                ...
                C_IMU_CT_FINAL: begin
                    if ($unsigned(sat_rshift_s64_rne(dir_sum, 30)) <= $unsigned(IMU_GEOMETRY_MIN2_Q30)) begin
                        qacc_available <= 1'b0;
                        selected_mode_l <= MODE_GYRO_ONLY;
                        // The current transaction has fallen back to gyro
                        // propagation because the tilt reference is
                        // geometrically singular.  Commit the same source
                        // mode atomically with qprev; otherwise the output
                        // would say GYRO_ONLY while mode_l incorrectly
                        // retained IMU_TILT for the next frame.
                ...
                // qmul1 = qyaw * qpitch.  The structured inputs require
                // four, rather than a generic sixteen, shared products.
                ...
                C_IMU_QMUL1_FINAL: begin
                    // qyaw and qpitch half-pairs can each reach two.  Their
                    // product therefore reaches four and must be uniformly
                    // rescaled before it re-enters the 32-bit shared
                    // multiplier.  The common /2 factor preserves the
                    // quaternion direction exactly.
                ...
                // qacc_pre = qmul1 * qroll.  The roll input also has only
                // w/x components, so this needs eight shared products.
